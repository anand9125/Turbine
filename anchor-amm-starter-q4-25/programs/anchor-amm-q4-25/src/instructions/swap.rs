...
use crate::{errors::AmmError, state::{Config, ConstantProduct, LiquidityPair}};
...
impl<'info> Swap<'info> {
    pub fn swap(
        ...
        let pair = if is_x_to_y {
            LiquidityPair::X
        } else {
            LiquidityPair::Y
        };

        let result = curve.swap(pair, amount_in, min_out)?;

        self.deposit_tokens(is_x_to_y, amount_in)?;

        self.withdraw_tokens(!is_x_to_y, result.withdraw)?;
